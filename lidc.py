# Copyright (C) 2017 Therapixel / Pierre Fillard (pfillardçtherapixel.com)
#
...
#################  log_device_placement=True antes
...
#import tensorflow.nn as batch_norm
# from tensorflow.keras.layers import BatchNormalization as batch_norm
from tensorflow.compat.v1.layers import batch_normalization as batch_norm
# import tf.nn.batch_normalization as batch_norm
from tensorflow.keras.layers import Dense, Conv2D, Flatten, BatchNormalization, MaxPool3D, Dropout
...
from tensorflow.python.ops.nn import relu, elu, relu6, sigmoid, tanh, softmax, avg_pool3d
...
def _variable_on_cpu(name, shape, initializer, regularizer=None):
    with tf.device('/cpu:0'):
        var = tf.compat.v1.get_variable(name, shape, initializer=initializer, regularizer=regularizer, dtype=tf.float32)
    return var
...
def convolution3d(input, num_outputs, kernel_size, strides, padding, activation_fn, is_training, 
                  ...
                  weights_initializer=tf.initializers.GlorotUniform(), bias_initializer=tf.zeros_initializer(), 
                  with_bias=True, with_bn=False, with_bn_in_place=True, with_bn_after_relu=False, scope=None):
    with tf.compat.v1.variable_scope(scope):
        try:
          num_input_channels = input.get_shape().as_list()[4]
        except:
          num_input_channels = input.input_shape()[4]
        ...
        if with_bn and not with_bn_after_relu:
            if with_bn_in_place:
                input = batch_norm(input, momentum=0.9, center=True, scale=False,
                                   name='batch_norm')
            else:
                input = batch_norm(input, momentum=0.9, center=True, scale=False,
                                   name='batch_norm')
        output = tf.nn.conv3d(input, filters=weights, strides=strides, padding=padding)
        if with_bias:
        ...
        if (activation_fn is not None):                        
            ...
            if with_bn and with_bn_after_relu:
                if with_bn_in_place:
                    output = batch_norm(output, momentum=0.9, center=True, scale=False
                                        ,name='batch_norm')
                else:
                    output = batch_norm(output, momentum=0.9, center=True, scale=False,
                                        name='batch_norm')
...
def _create_base_network(input, is_training, with_bn=False):
    ...
    input = MaxPool3D(pool_size=(2,2,2), strides=(2,2,2), padding='VALID', name="pool11")(input) 
    ...
    input = MaxPool3D(pool_size=(2,2,2), strides=(2,2,2), padding='VALID', name="pool31")(input)    
    ...
    input = MaxPool3D(pool_size=(2,2,2), strides=(2,2,2), padding='VALID', name="pool41")(input)
    ...
    input = Dropout(rate=0.5, name="dropout61")(input)
    ...
    input = Dropout(rate=0.5, name="dropout71")(input)
...
def _create_base_network_all_scores(input, is_training, padding='VALID'):
    ...
    input = MaxPool3D(pool_size=(2,2,2), strides=(2,2,2), padding=padding, name="pool1")(input)
    ...
    input = MaxPool3D(pool_size=(2,2,2), strides=(2,2,2), padding=padding, name="pool2")(input)
    ...
    input = MaxPool3D(pool_size=(2,2,2), strides=(2,2,2), padding=padding, name="pool3")(input)
    ...
    input = MaxPool3D(pool_size=(2,2,2), strides=(2,2,2), padding=padding, name="pool4")(input)
            
    # FC layers as convolutional
    input = Dropout(rate=0.5, name="dropout_0")(input)
    ...
    input = Dropout(rate=0.5, name="dropout_1")(input)
...
def _create_base_network_emphyseme(input, is_training):
    ...
    input = batch_norm(input, momentum=0.9, center=True, scale=False, name='batch_norm_0')
...
def inference_all_scores(input, num_outputs, is_training):
    ...
    with tf.compat.v1.variable_scope('final'):
        input_bn = batch_norm(input, momentum=0.9, center=True, 
                             scale=False, name='batch_norm')
        if num_outputs==9:
            output = []
            for i in range(num_outputs):
                preds = Dropout(rate=0.5, name="dropout_%d"%i)(input_bn)
            ...
            preds = Dropout(rate=0.5, name='dropout')(input_bn)
...
def inference_emphyseme(input, is_training, num_outputs=3):
    ...
    with tf.compat.v1.variable_scope('final'):
        input = Dropout(rate=0.5, name='dropout')(input)
...
def scan_volume(volume, model, with_bn=False):
    ...
    with tf.Graph().as_default(), tf.device('/gpu:0'):
        ...
        x_pl = tf.compat.v1.placeholder(tf.float32, shape=(1, None, None, None, 1), name='data_x')
        ...
        gpu_opts = tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction=1.0)
...
def scan_volume_emphyseme(volume, model):
    ...
    with tf.Graph().as_default(), tf.device('/gpu:0'):
        ...
        # initialize the Session
        sess = tf.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_opts, 
                                                allow_soft_placement=True, 
                                                log_device_placement=False))
        sess.run(tf.compat.v1.global_variables_initializer())
...
def refine_scoremap(volume, positions, radius, model_list, max_ite=3, with_bn=False):
    ...
    with tf.Graph().as_default(), tf.device('/gpu:0'):
        ...
        # Setting up placeholder, this is where your data enters the graph!
        x_pl = tf.compat.v1.placeholder(tf.float32, shape=(None, 48, 48, 48, 1), name='data_x')
...
def estimate_scores(patches, num_outputs, model, batch_size=8):
    ...
    with tf.Graph().as_default(), tf.device('/gpu:0'):
        ...
        # Setting up placeholder, this is where your data enters the graph!
        x_pl = tf.compat.v1.placeholder(tf.float32, shape=(None, patch_size, patch_size, patch_size, 1), name='data_x')
